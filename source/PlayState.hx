...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
            ...
            case 'street':
                var bg:BGSprite = new BGSprite('street', -400, -400);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:Int = checkForAchievement([1, 2, 3]);
			if(achieve > -1) {
	...
	private function checkForAchievement(arrayIDs:Array<Int>):Int {
		for (i in 0...arrayIDs.length) {
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					case 1:
						if(Paths.formatToSongPath(SONG.song) == 'really-happy') {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
					case 2:
						if(Paths.formatToSongPath(SONG.song) == 'playtime') {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
					case 3:
						if(Paths.formatToSongPath(SONG.song) == 'really-happy-playtime') {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
				}
